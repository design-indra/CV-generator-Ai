exports.handler = async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      return { statusCode: response.status, headers, body: JSON.stringify({ error: data.error?.message || 'Groq API error' }) };
    }

    // Format response agar sama dengan format Anthropic
